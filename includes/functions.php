...
// checks to see if username is allowed
...
// check very basic phone number regex, not very useful but
// will make it so that only digits can be entered
...
// allowing gender to be written, but no symbols apart from -
...
// check to see if user exists (checks username, email and phone number)
function uidExists($conn, $username, $email, $phone)
{
    ...
    //if the sql or connection fail to work, get a statement failed error
...
function createUser($conn, $name, $surname, $email, $username, $pwd, $gender, $phone, $creationDate)
{
    // ? placeholders for user first name,user Last name,userEmail,userUid,userPWD, user gender an user phone number
    $sql = "INSERT INTO users (userFname,userLname,userEmail,userUid,userPwd,userGender,userPhone, creationDate) VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: ../signup.php?error=statementfailed");
        exit();
    ...
    mysqli_stmt_bind_param($stmt, "ssssssss", $name, $surname, $email, $username, $hashedPwd, $gender, $phone, $creationDate);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
